...
import type { Category, Condition, HandoverMethod, Listing } from "@/lib/types";
...
export function SearchClient() {
  ...
  useEffect(() => {
    ...
    handovers.forEach((h) => params.append("handover", h));
    apiGet<{ listings: Listing[]; total: number }>(`/api/listings?${params}`).then((r) => {
      setResults(r.listings);
      setTotal(r.total);
    });
  }, [query, category, maxPrice, conditions, handovers, page]);
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="label-mono mt-0.5 truncate text-[10px] text-text-label">
                      {conditionLabel(listing.condition)} · {listing.seller.name}
                    </p>
