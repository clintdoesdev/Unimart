...
function CartContent() {
  ...
  function load() {
    apiGet<{ groups: CartGroup[] }>("/api/cart").then((r) => setGroups(r.groups));
  }

  useEffect(load, []);

  async function setQty(listingId: string, qty: number) {
    await apiPatch(`/api/cart/${listingId}`, { qty });
    load();
    refreshCart();
  }

  async function removeItem(listingId: string) {
    await apiDelete(`/api/cart/${listingId}`);
    load();
    refreshCart();
  }
    ...
    <div className="flex flex-1 flex-col pb-40 lg:pb-8">
      ...
      <div className="flex flex-col gap-6 px-5 py-5 lg:mx-auto lg:w-full lg:max-w-2xl lg:py-8">
        {groups.map((group) => (
          <div key={group.sellerId}>
            <p className="label-mono mb-2.5 text-[11px] text-text-label">
              SOLD BY {group.seller.name.toUpperCase()}
            </p>
              ...
              {group.lines.map((line) => (
                <div key={line.listingId} className="flex items-center gap-3 p-3.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[15px]">{line.title}</p>
                    <p className="mt-0.5 text-accent">₹{line.price}</p>
                  </div>
        ...
        <div className="rounded-[20px] bg-accent-tile p-3.5">
          {discount > 0 ? (
            <p className="text-sm text-accent">Student code applied −₹{discount}</p>
          ) : (
